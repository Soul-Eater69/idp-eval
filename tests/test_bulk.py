...
def test_list_output_stays_one_case_and_is_rendered():
    ...
    result = EvaluationFramework(
        evaluators=[SourceCoverageEvaluator(judge)]
    ).evaluate(case)
...
def test_evaluate_many_independent_results_in_order():
    framework = EvaluationFramework(
        evaluators=[SourceCoverageEvaluator(ReusableSourceJudge())]
    )
    results = framework.evaluate_many(_cases())
    assert isinstance(results, list) and len(results) == 2
    assert results[0]["source_coverage"].score == 1.0
    assert results[1]["source_coverage"].score == 1.0
...
def test_evaluate_many_excel_rows_attributable(tmp_path):
    ...
    assert all(r["metric"] == "source_coverage" for r in rows)


def test_evaluate_many_metric_subset_applied_to_every_case():
    framework = EvaluationFramework(
        evaluators=[
            SourceCoverageEvaluator(ReusableSourceJudge()),
            TaskCoverageEvaluator(object()),  # not selected -> never called
        ]
    )
    results = framework.evaluate_many(_cases(), metrics=["source_coverage"])
    assert all(set(r) == {"source_coverage"} for r in results)
...
def test_evaluate_groups_reuses_evaluate_many(monkeypatch):
    framework = EvaluationFramework(
        evaluators=[SourceCoverageEvaluator(ReusableSourceJudge())]
    )
    seen = {}
    original = framework.evaluate_many

    def spy(cases, **kwargs):
        seen["cases"] = list(cases)
        return original(seen["cases"], **kwargs)

    monkeypatch.setattr(framework, "evaluate_many", spy)
    framework.evaluate_groups(_groups())
    # Grouped API delegated to evaluate_many with 3 fanned-out cases.
    assert [c.case_id for c in seen["cases"]] == [
        "theme-1:0", "theme-1:1", "theme-2:0",
    ]
    assert all(isinstance(c, EvaluationCase) for c in seen["cases"])
